...
resource "azurerm_template_deployment" "Chatbot-app-svc" {
  ...
  parameters = {
    ...
    "zipUrl"              = "${var.zipUrl}"
    "serverFarmId"        = "${var.plan_id == "" ? azurerm_app_service_plan.Chatbot-serviceplan[0].id : var.plan_id}" //"/subscriptions/${data.azurerm_subscription.current.id}/resourceGroups/${var.resourceGroupName}/providers/Microsoft.Web/serverfarms/ScDc-CIOCPS-StudentChatbot-app-svcplan"
